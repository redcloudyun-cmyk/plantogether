...
function TopBar({ screen }: { screen: ScreenId }) {
  ...
  const heading = screen === 'workspace' ? title : screen === 'activity' ? '3. Activity' : screen === 'settings' ? '4. Settings / WebMCP' : 'Dashboard';
    ...
    <div className="h-20 flex-shrink-0 bg-white border-b border-border px-7 flex items-center justify-between">
      <div>
        ...
        <p className="text-xs text-text-tertiary mt-1">
          {screen === 'activity' ? 'Real-time activity and audit trail of human and agent actions.' : screen === 'settings' ? 'Manage connection, tools, permissions, autonomy and AI context.' : 'Human + Agent collaborative planning'}
        </p>
      </div>
      <div className="flex items-center gap-3">
        <span className="hidden lg:flex items-center gap-2 border border-border rounded-lg px-3 py-2 text-xs font-medium">
          ...
          WebMCP <span className={webmcpAvailable ? 'text-emerald-600' : 'text-text-tertiary'}>{webmcpAvailable ? 'Connected' : 'Unavailable'}</span>
        </span>
        {screen === 'workspace' && (
          <select value={autonomyMode} onChange={(event) => setAutonomyMode(event.target.value as typeof autonomyMode)} className="border border-border rounded-lg bg-white px-3 py-2 text-xs font-medium">
            <option value="observe">Observe Mode</option>
